...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        System.Timers.Timer updateTimer = new System.Timers.Timer(10000); // Update every 10 seconds
    ...
    static List<JsonPatchDocument> CreatePatchList(List<(DateTime timestamp, string position, double x, double y, double angle)> positionList)
    {
        ...
        foreach (var item in positionList)
        {
            ...
            patch.AppendAdd("/position", item.position);
            patch.AppendAdd("/x", item.x.ToString());
            patch.AppendAdd("/y", item.y.ToString());
            patch.AppendAdd("/angle", item.angle.ToString());
